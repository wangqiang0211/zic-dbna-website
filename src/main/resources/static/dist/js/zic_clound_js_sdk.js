...
function zic_clound(userid, appkey, secretkey) {
    var obj = new Object();
    obj.deviceName = userid,
        obj.productKey = appkey,
        obj.deviceSecret = secretkey,
        obj.clientId = Math.random().toString(36).substr(2),
        obj.timestamp = (new Date()).valueOf(),
        obj.mqttClientId = obj.clientId + "|securemode=2,signmethod=hmacsha1,timestamp=" + obj.timestamp + "|",
        obj.content = 'clientId' + obj.clientId + 'deviceName' + obj.deviceName + 'productKey' + obj.productKey + 'timestamp' + obj.timestamp,
        obj.mqttUsername = obj.deviceName + "&" + obj.productKey,
        obj.mqttPassword = CryptoJS.HmacSHA1(obj.content, obj.deviceSecret).toString();
    ...
    function connection(timeout, callback) {
        var option = {
            useSSL: true,
            keepAliveInterval: 80,
            userName: obj.mqttUsername,
            password: obj.mqttPassword,
            onSuccess: callback.onConnectionSucc,
            onFailure: callback.onConnectionFail
        };
        option.timeout = timeout;
        client = new Paho.MQTT.Client("2Cbw639wXUf.iot-as-mqtt.cn-shanghai.aliyuncs.com", 443, obj.mqttClientId);
        client.onConnectionLost = callback.onConnectionLost;
        client.onMessageArrived = callback.onMessageArrived;
        if (callback.onMessageDelivered != undefined) {
            client.onMessageDelivered = callback.onMessageDelivered
        }
        client.connect(option);
    }


    function topicTool(topicC) {
        var topicRS = topicC.replace(/\s+/g, "");
        if (topicRS.indexOf("&") != -1) {
            return topicRS.split("&");
        } else {
            return "fail";
        }
    }
    ...
    function sendMSG(topicC, msg) {
        var topic = topicTool(topicC);
        if (topic != "fail") {
            ...
            message.destinationName = "/" + obj.productKey + "/" + obj.deviceName + "/" + topic[1];
            client.send(message);
        }
    }

    function subscriptions(topicC, callback) {
        var topic = topicTool(topicC);
        if (topic != "fail") {
            client.subscribe("/broadcast/" + obj.productKey + "/" + topic[0] + "&" + topic[1], {
                onSuccess: function () {
                    var pp = {
                        topic: topicC,
                        status: "success"
    ...
    function subscription(topicC, callback) {
        var topic = topicTool(topicC);
        if (topic != "fail") {
            client.subscribe("/" + obj.productKey + "/" + obj.deviceName+ "/" + topic[1], {
                onSuccess: function () {
                    var pp = {
                        topic: topicC,
                        status: "success"
    ...
    function unsubscription(topicC, callback) {
        var topic = topicTool(topicC);
        if (topic != "fail") {
            client.unsubscribe("/" + obj.productKey + "/" + obj.deviceName + "/" + topic[1], {
                onSuccess: function () {
                    var pp = {
                        topic: topicC,
                        status: "success"
    ...
    function unsubscriptions(topicC, callback) {
        var topic = topicTool(topicC);
        if (topic != "fail") {
            client.unsubscribe("/broadcast/" + obj.productKey + "/" + topic[0] + "&" + topic[1], {
                onSuccess: function () {
                    var pp = {
                        topic: topicC,
                        status: "success"
